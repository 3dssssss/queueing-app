...
<body class="bg-gray-100 text-gray-900 tracking-wider leading-normal">
    ...
    <div class="min-h-screen flex items-center justify-center bg-gray-50 mt-20">
        ...
        <div class="bg-pink-200 shadow-lg rounded-lg p-8 w-full sm:w-3/4 md:w-2/3 lg:w-1/2 xl:w-1/3 text-center">
            
            <h1 class="text-3xl font-extrabold text-gray-800 mb-6">PLEASE WAIT IN LINE</h1>

            <!-- Profile Image -->
            <img class="mx-auto mb-6 rounded-full w-32 h-32 object-cover" src="{{ asset('images/undraw_wait-in-line.svg') }}" alt="Wait in line">

            <!-- Title -->
            <h1 class="text-2xl font-extrabold text-gray-800 mb-6">Your Ticket Numbers</h1>

            <!-- Ticket Number Display -->
            <div class="space-y-4">
                @foreach($tickets as $ticket)
                    <p class="text-4xl font-extrabold text-pink-600 bg-pink-100 py-4 rounded-md shadow-lg">
                        {{ $ticket->ticket_number }}
                        <p class="text-large font-extrabold text-blue-600 bg-blue-100 py-4 rounded-md shadow-lg">
                        Created At: {{ $ticket->created_at->format('Y-m-d H:i:s') }} <!-- Display creation date and time -->
                        </p>
                        <p class="text-large font-extrabold text-blue-600 bg-blue-100 py-4 rounded-md shadow-lg">
                        Expires At: {{ $ticket->expires_at ? $ticket->expires_at->format('Y-m-d H:i:s') : 'N/A' }} <!-- Display expiration date and time -->
                        </p>
                    </p>     
                @endforeach
            </div>
